...
* `get_acoustic_detections()` now uses a different protocol to retrieve data. It can now reliably return 10M+ detections without timeouts.
* `get_acoustic_detections()` now returns a progress bar for large queries.
* `get_acoustic_detections()` now has a `deployment_id` filter argument.
* `get_acoustic_detections()` now has a `tag_serial_number` argument, which is more reliable than `acoustic_tag_id` (which is still supported). Thanks @lottepohl for the suggestion.
* `get_acoustic_detections()` may return fewer (erroneous) detections than before, due to fixes in the database.
* `get_animals()` now includes `type_type = "archival"` data.
